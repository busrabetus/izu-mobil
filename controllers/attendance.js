...
// Belirli bir ders için devamsızlık detayları
exports.getAttendanceDetails = (req, res) => {
    ...
    const query = `
        SELECT 
            c.class_name as ders_adi,
            p.date as tarih,
            p.presence_count as katilim,
            p.presence_max as toplam
        FROM user u
        JOIN student s ON u.user_id = s.user_id
        JOIN enrollments e ON s.student_id = e.student_id
        JOIN classgroup cg ON e.group_id = cg.group_id
        ...
        JOIN presence_student ps ON e.enro_id = ps.enro_id
        JOIN presence p ON ps.presence_id = p.presence_id
        WHERE u.user_id = ? AND c.class_id = ?
        ORDER BY p.date DESC;
    `;

    db.query(query, [user_id, classId], (err, results) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }

        if (results.length === 0) {
            return res.status(404).json({ message: "Devamsızlık detayları bulunamadı" });
        }
